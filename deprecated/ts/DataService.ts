...
module ap {
    ...
    var service:DataService, $q:ng.IQService, $timeout, $http:ng.IHttpService, apConfig, apUtilityService,
    ...
    export class DataService {
         ...
         */
        createListItem(model:Model, listItem:ListItem, options?:{buildValuePairs:boolean; valuePairs:[ [string, any][] ]}):ng.IPromise<IListItem> {
            var defaults = {
                    batchCmd: 'New',
         ...
         */
        deleteListItem(model:Model, listItem:ListItem, options?:{target:ICache}) {
            var defaults = {
         ...
         */
        executeQuery(model:Model, query:IQuery, options?:{target:ICache;}) {
         ...
         */
        processChangeTokenXML(model:Model, query:IQuery, responseXML:XMLDocument, opts) {
         ...
         * @returns {object} Promise which resolves with the newly created item.
         */
        updateListItem(model:Model, listItem:ListItem, options) {
            var defaults = {
                    batchCmd: 'Update',
                    buildValuePairs: true,
                    ID: listItem.id,
